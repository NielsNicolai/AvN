...
start_date = "30 October 2019"
end_date = "1 November 2019"

#Define the name and location of the files generated by the controller
files_ctrl = ['HTML reports/191105/intermDataAvNCtrl_DO_20191104_30-01.csv'] #['data/October/intermDataAvNCtrl_fAE_20191020.csv']
...
range_influent = [22, 30]#[10, 50]
range_effluent = [0,30]#[0, 20]
range_AvN = [-1,10]
range_aeration = [0.0, 1.2]#[0.0, 2.00]
range_airflow = [0, 400]#[0.0, 1000]
...
if files_ctrl:
    ...
    df_db_ctrl = df_db.join(df_ctrl, how='outer')
else:
    df_db_ctrl = df_db
...
# Calculate the average DO for the Pilote
df_avg_DO4 = calc_average(df['Pilote reactor 4 DO'],1)
df = df.join(df_avg_DO4, how='outer')

# Calculate the average DO for the Pilote
df_avg_DO5 = calc_average(df['LDO250'],1)
df = df.join(df_avg_DO5, how='outer')

# Calculate the average air flow for the Pilote
df_avg_AIR = calc_average(df['Pilote reactor 4 Flowrate (Gas)'],200)
df = df.join(df_avg_AIR, how='outer')
...
if reactor == 'Copilote':
    ...
    fig.add_trace(
        go.Scattergl(
            x=df.index,
            y=df[reactor+' reactor 4 DO - avg cycle'],
            name = 'DO tank 5',
            mode='lines+markers',
            legendgroup='leg4',
            marker=dict(
...
if reactor == 'Pilote':           
    fig.add_trace(
        go.Scattergl(
            x=df.index,
            y=df[reactor+' reactor 4 Flowrate (Gas) - avg cycle'],
