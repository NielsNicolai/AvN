...
try:    
    ...
    fAE_1  = stored_vals['fAE_1'].iloc[-1]
    ...
    i = 2
    if np.isnan(fAE_1) or np.isnan(error_1):
           fAE_1  = stored_vals['fAE_1'].iloc[-i]
...
Nfltr = 5*Fs #filter length in minutes
...
elif error_mode == "diff":
    error = NH4 - (alpha*NO3) - beta #difference
else:
    error = NH4 - (alpha*NO3) - beta #difference
  
fAE = fAE_1 + (error*c0 + error_1*c1 + error_2*c2) #note the negative gain of the process

#Clipping of the control action according to the cycle limits
fAE_min = 0 
fAE_max = 1
fAE = np.clip(fAE, a_min=fAE_min, a_max=fAE_max)

fAE_man = 0.5 #Manual mode setpoint
if operating_mode == "auto":
    fAE = fAE
else:
    fAE = fAE_man
	
if np.isnan(fAE): #If for some reason the calculated value is NaN
    fAE = fAE_1

#%% APPLY SETPOINT
#Overwrite CSV file DO setpoints continuous DO control
DOsp = 2
cycle_duration = 5 #minutes

write_delay_1 = 1 #minutes
write_delay_2 = write_delay_1+(1-fAE)*cycle_duration #minutes
write_delay_3 = cycle_duration+(write_delay_1/2) #minutes

write_time_1 = datetime.datetime.now() + datetime.timedelta(minutes=write_delay_1)
write_time_2 = datetime.datetime.now() + datetime.timedelta(minutes=write_delay_2)
write_time_3 = datetime.datetime.now() + datetime.timedelta(minutes=write_delay_3)

new_fAE = pd.DataFrame(
    data={
        'date':[datetime.datetime.now().strftime("%Y.%m.%d"), datetime.datetime.now().strftime("%Y.%m.%d"), datetime.datetime.now().strftime("%Y.%m.%d")], 
        'hour':[write_time_1.strftime("%H:%M:%S"), write_time_2.strftime("%H:%M:%S"), write_time_3.strftime("%H:%M:%S")],
        'fAE':[round(0,2), round(DOsp,2), round(DOsp,2)],
        }
    )

with open(path_ctrlAction+'AIC_241_Data.csv', 'a', newline='') as f:
